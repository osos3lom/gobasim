...
confirmation-gated writes couldn't self-correct malformed model args — since **resolved** via a
required-args validation gate (see [`M9-CHECKLIST.md`](M9-CHECKLIST.md)).
...
- **Missing:** M9 sign-off against a **deployed** `mshalia` + the real-WhatsApp voice round-trip (the
  ...
  `high` tier; identity-resolution cache. (F-1 confirmation-write self-correction is resolved.)
...
|---|---|---|---|---|
| D-1 | Real secrets loaded from a static file, not fetched from the vault | Security | High | All 12 values now exist in **GCP Secret Manager** and the VM's service account can read them (`roles/secretmanager.secretAccessor` + `cloud-platform` scope granted 2026-07-13). The `ExecStartPre` fetch script is now **drafted** — [`gcp/fetch-secrets.sh`](../gcp/fetch-secrets.sh), wired into [`gcp/sawt.service`](../gcp/sawt.service) (`gcp/README.md` §5a) — but **not yet applied to the live VM**, and its secret-id names are a naming guess that must be checked against what was actually created in Secret Manager before it's trusted. Remaining: verify secret ids, deploy the script + updated unit file to `gateway-go`, confirm `/opt/sawt/.env` is written fresh at boot, then rotate the values that were ever written to the old static file. |
...
| ~~F-1~~ | ~~Confirmation-gated writes can't self-correct bad model args~~ | Functionality | **Resolved** | `enforceRequiredArgs` (`internal/workflow/clarification.go`, wired into the tool loop in `engine.go` ahead of the risk gate) validates a parsed tool call against the tool's own required-fields schema **before** it's parked for confirmation, auto-derives fields where configured (e.g. `nameEn` transliterated from `nameAr`), and durably asks the user for anything still missing across turns instead of freezing bad args. Covered by `TestEnforceRequiredArgs_*` in `clarification_test.go`, including the exact `register_horse` scenario from the finding. Landed in the same commit as this checklist (2026-07-13) — see M9-CHECKLIST F-1. |
...
- **1a. Rotate & vault all production secrets** — P0 · 0.5 day remaining. All 12 values now live in
  **GCP Secret Manager**, and the VM SA has `secretmanager.secretAccessor` + the `cloud-platform`
  scope (granted 2026-07-13). The `ExecStartPre` fetch script is **drafted**
  ([`gcp/fetch-secrets.sh`](../gcp/fetch-secrets.sh) + [`gcp/sawt.service`](../gcp/sawt.service),
  `gcp/README.md` §5a) but not yet applied to the live VM. **Remaining:** confirm the script's
  secret-id names match what's actually in Secret Manager, deploy it + the updated unit file to
  `gateway-go`, verify `/opt/sawt/.env` is written fresh at boot, then rotate the values that were
  ever written to the old static file (Neon password, `ADMIN_PASSWORD`, `SESSION_SECRET`, API keys).
...
**Security & Secrets**
- [ ] All secrets rotated; boot path switched from static `.env` to a GCP Secret Manager fetch (P0) — values are in Secret Manager and the VM SA can read them; the `ExecStartPre` fetch script is drafted (`gcp/fetch-secrets.sh`) but not yet deployed to the VM; rotation is still open
...
- **Resolved:** F-1 (confirmation-gated writes couldn't self-correct malformed model args) — fixed by
  the required-args validation gate in `internal/workflow/clarification.go`.
